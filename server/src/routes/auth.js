...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ message: 'Verified', token: jwt, user: { id: user._id, role: user.role, fullName: user.fullName } });
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!token || token.expiry < new Date()) return res.status(410).json({ error: 'OTP expired' });
      if (!verifyOtpHash(otp, token.hash)) {
        await recordOtpFailure(user);
        await maybeSendLockoutEmail(user, sendTemplate);
        if (isLocked(user)) {
      ...
      res.json({ token: jwt, user: { id: user._id, role: user.role, fullName: user.fullName, studentId: user.studentId } });
    } catch (err) {
